        ...
        .factory('clipboard', ['$document', '$window', function ($document, $window) {
            ...
            function copyNode(node, str) {
                ...
                try {
                    ...
                    try {
                        if (str) $document[0].addEventListener("copy", listener);
                        var res = $document[0].execCommand('copy');
                        if (str) $document[0].removeEventListener("copy", listener);
                        if (!res) {
                            throw ('failure copy');
                        }
                    } finally {
            ...
            function copyText(text, context, copyAsHTML) {
                ...
                copyNode(node, copyAsHTML ? text : null);
            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    element.on('click', function (event) {
                        try {
                            clipboard.copyText(scope.text, element[0], 'copy-as-html' in attrs);
